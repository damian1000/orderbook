...
class TestSingleWriterOrderBook : OrderBookContractTest() {
    override fun newOrderBook(): OrderBook = SingleWriterOrderBook()

    /**
     * Many threads submit concurrently, but the single writer must apply every
     * operation serially — so a balanced add-then-remove leaves an empty book
     * with nothing lost or duplicated.
     */
    @Test
    @Timeout(value = 10, unit = TimeUnit.SECONDS)
    fun concurrentSubmissionsAreSerialisedWithoutLoss() {
        SingleWriterOrderBook().use { book ->
            val threads = 8
